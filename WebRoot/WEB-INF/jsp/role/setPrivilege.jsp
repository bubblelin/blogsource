		...
		<script type="text/javascript">
			$(function(){
				...
				$('input[name=privilegeIds]').click(function(){
					//1.当选中某个权限时，同时选中其下级权限，当取消某个权限时，同时取消其下级权限
					$(this).siblings('ul').find('input').attr('checked',this.checked);
					//2.当选中某个权限时，同时选中其直接上级权限
					if(this.checked){
						$(this).parents('li').children('input').attr('checked',true);
					}else{
						//3.当取消某个权限时，如果同级权限都没有被选中，同时取消上级权限 
						var nSize = $(this).parent('li').siblings('li').children('input:checked').size();
						if(nSize == 0){
								...
								<ul id="privilegeTree">
									...
									<li>
										<input id="cb_${id}" 
											...
											type="checkbox" value="${id}" name="privilegeIds"/>

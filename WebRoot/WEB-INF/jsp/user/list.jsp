...
<div id="MainArea">
        ...
        <s:iterator value="listData">
            <tr class="TableDetail1 template">
                ...
                <s:if test="#session.loginUser.hasPrivilegeByName('用户删除')">
                	<s:a onClick="return window.confirm('确定删除这条记录吗？')" action="user_delete?id=%{ id }" namespace="/">删除</s:a>
                </s:if>
                ...
                <s:if test="#session.loginUser.hasPrivilegeByName('用户初始化密码')">
					<s:a action="user_initPassword?id=%{id}" namespace="/" onClick="return window.confirm('您确定要初始化密码为1234吗？')">初始化密码</s:a>
                </s:if>
                </td>
            </tr>
        </s:iterator>
    ...
    <div id="TableTail">
        ...
        <s:if test="#session.loginUser.hasPrivilegeByName('用户添加')">
            <s:a action="user_addUI?id=%{id}"><img src="${pageContext.request.contextPath}/style/images/createNew.png" /></s:a>
        </s:if>
        </div>
    </div>
</div>
